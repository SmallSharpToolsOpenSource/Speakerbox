...
final class ProviderDelegate: NSObject {
    ...
    func reportIncomingCall(uuid: UUID, handle: String, hasVideo: Bool = false, completion: ((NSError?) -> Void)? = nil) {
        if #available(iOS 10.0, *) {
            ...
            guard let provider = provider as? CXProvider else {
                return
            }
...
extension ProviderDelegate: CXProviderDelegate {
    ...
    func provider(_ provider: CXProvider, perform action: CXStartCallAction) {
         ...
         */
        call.hasStartedConnectingDidChange = { [weak provider] in
            provider?.reportOutgoingCall(with: call.uuid, startedConnectingAt: call.connectingDate)
        }
        call.hasConnectedDidChange = { [weak provider] in
